...
## IMPORTANT: Slice Naming Updates

The specification below references slice names that have been updated since the initial design. Here's the mapping:

| Specification Name | Actual Implementation |
|--------------------|----------------------|
| `createInitialOrderSlice.ts` | `createDefaultsSlice.ts`, `createUserPrefsSlice.ts`, `createFdc3IntentSlice.ts` |
| `createComputedSlice.ts` | `createDerivedSlice.ts` |

The **layered state pattern remains unchanged** - only the names and organization were refined during development. See `CODE_CHANGES.md` for the full refactoring history.
...
export const ORDER_TYPES = {
  TAKE_PROFIT: {
    ...
    STOP_LOSS: {
        fields: ['direction', 'liquidityPool', 'notional', 'limitPrice', 'timeInForce', 'account'],
        validationSchema: StopLossrderSchema,
        initialFocus: 'limitPrice' // Field to focus when this type is selected
    },
    FLOAT: {
        fields: ['direction', 'liquidityPool', 'notional', 'account'],
        validationSchema: FloatOrderSchema,
